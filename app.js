...
// CÁLCULO AUTO DEL PERSONAL
$$('#aud-auditado-list').forEach(el => {
    el.addEventListener('change', () => {
        let count = $$('#aud-auditado-list input:checked').length;
        if($('aud-personal')) $('aud-personal').value = count;
    });
});
...
window.renderF020 = () => {
    ...
    let auditadosOptions = `<option value="">-- Sel --</option>` + (selectedAuditData.auditado?selectedAuditData.auditado.split(', ').map(a=>`<option value="${a}">${a}</option>`).join(''):'');
    currentAuditF020.forEach((i, idx) => {
        ...
        let aOpt = `<option value="${i.auditado||''}" selected>${i.auditado||'-- Sel --'}</option>` + auditadosOptions;
