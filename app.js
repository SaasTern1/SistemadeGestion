...
window.cambiarVista = (id, btn) => {
    ...
    const section = $(id); if(section) section.classList.add('active'); 
    if(btn) btn.classList.add('active');
    if(window.innerWidth <= 768) { 
        const sidebar = $('sidebar'); if(sidebar) sidebar.classList.remove('open'); 
        const overlay = $('sidebar-overlay'); if(overlay) overlay.classList.remove('active'); 
    }
};
...
window.getNextFCI = async () => {
    ...
    await runTransaction(db, async (t) => { 
        const snap = await t.get(ref); let count = 1; 
...
window.verificarAlertasAuditoria = (auditoriasArray) => {
    ...
    auditoriasArray.forEach(a => {
        ...
        if(asunto) { 
            window.sendNotification({to: globalAuditPlan.correos.join(',')}, asunto, `Se aproxima una auditoría programada para el ${window.formatearFechaAbreviada(a.fecha)} en ${a.lugar}.\n\nRequisitos a evaluar: ${a.requisitos}\nAuditados: ${a.auditado}\nAuditores: ${a.auditor}.`); 
        }
    });
};
...
window.cargarDatosCentrales = () => {
    ...
    onSnapshot(doc(db, "artifacts", appId, "public", "data", "Configuracion", "NormaOEA"), (docSnap) => {
        if(docSnap.exists()) { 
            ...
            requisitosOEA = []; manualOEA = { url: "", nombre: "" }; 
        } 
...
window.verDetalle = async (id) => {
    ...
    for(let i=1; i<=4; i++) { const st = $('s'+i); if(st) { st.className = 'step'; if(cnc) continue; if(i <= s.idx) st.classList.add('completed'); if(i === s.idx + 1 && !apr) st.classList.add('active'); } }

    const esAdminSGC = p.admin || p.p_gest_sgc; const esGer = p.p_ger_apr && currentUser.gerencias && currentUser.gerencias.includes(s.gerencia); const activo = !apr && !cnc;
...
window.renderF020 = () => {
    ...
    currentAuditF020.forEach((i, idx) => {
        let dis=canEd?'':'disabled', rOpt=`<option value="">-- Sel --</option>`+requisitosOEA.map(r=>`<option value="${r}" ${i.requisito===r?'selected':''}>${r}</option>`).join('');
