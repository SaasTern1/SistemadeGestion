...
window.renderTablaMaestro = () => {
    ...
    dataSort.forEach(item => {
        ...
        columnasMaestro.forEach(col => {
            ...
            if(cType === 'url' || val.toString().startsWith("http")) { let fName = item['Nombre del documento'] || "Ver Documento"; let dUrl = window.getDownloadUrl(val); rowHTML += `<td><a href="${dUrl}" target="_blank" class="file-link">📁 Ver Archivo</a></td>`; } 
...
window.verDetalle = async (id) => {
    ...
    let adjOrigName = s.adjunto_nombre || "Ver Archivo"; if(adjOrigName.startsWith('http')) { try { adjOrigName = decodeURIComponent(adjOrigName.split('/').pop().split('?')[0]); } catch(e){} }
    ...
    if (isAprobadoFinalModal) {
        if (s.version_final) {
            ...
            let finName = s.documento_final_nombre || "Ver Documento Oficial"; if(finName.startsWith('http')) { try { finName = decodeURIComponent(finName.split('/').pop().split('?')[0]); } catch(e){} }
    ...
    const cb = document.getElementById('chat-box'); cb.innerHTML = s.chat ? s.chat.map(c => `<div class="chat-msg" style="border-left-color:${c.u===currentUser.nombre?'var(--primary)':'#cbd5e1'}"><b style="font-size:10px">${c.u}</b> <span style="font-size:9px;color:#94a3b8">${c.t}</span><br>${c.m}${c.archivo ? `<br><a href="${window.getDownloadUrl(c.archivo)}" target="_blank" style="font-size:10px;color:blue">Ver Adjunto</a>` : ''}</div>`).join('') : ''; document.getElementById('modal').style.display = 'flex';
};
