...
window.renderTablasSolicitudes = () => {
    ...
    if($('dash-mis-tot')) {
        ...
        $('dash-mis-tot').innerText = m.length; $('dash-mis-pend').innerText = m.filter(s => !s.estado.includes('Aprobado') && s.estado !== 'Anulado' && s.estado !== 'Rechazado').length; $('dash-mis-ok').innerText = m.filter(s => s.estado.includes('Aprobado Final')).length; $('dash-mis-rech').innerText = m.filter(s => s.estado === 'Anulado' || s.estado === 'Rechazado').length;
    }
...
window.completarLoginUI = () => {
    ...
    const p = currentUser.permisos || {}; setDisplay('admin-only', (p.admin || p.p_users || p.p_struct) ? 'block' : 'none'); setDisplay('nav-audit-group', (p.admin || p.p_audit_ver || p.p_audit_admin || p.p_audit_auditor || p.p_audit_dueno) ? 'block' : 'none'); setDisplay('nav-listado', (p.admin || p.p_ver_listado) ? 'flex' : 'none'); setDisplay('nav-all', (p.admin || p.p_ver_todas) ? 'flex' : 'none'); setDisplay('nav-noconf', (p.admin || p.p_gest_sgc) ? 'flex' : 'none'); setDisplay('nav-gest', (p.admin || p.p_gest_sgc || p.p_ger_apr) ? 'flex' : 'none');
    ...
    window.cargarDatosCentrales(); if($('nav-dash')) window.cambiarVista('sec-dash', $('nav-dash'));
};
...
window.cargarUsuarioParaEditar = (id) => {
    const u = allUsers.find(x => x.usuario === id); if(!u) return; $('user-form-title').innerHTML = `<span class="material-icons-round">edit</span> Editando: ${u.usuario}`;
    ...
    ['p-solicitar','p-ver-propias','p-ver-ger','p-ver-todas','p-paso1','p-paso2','p-paso4','p-gest-sgc','p-ger-apr','p-users','p-struct','p-ver-listado','p-audit-ver','p-audit-admin','p-audit-auditor','p-audit-dueno'].forEach(i => { let k = i.replace(/-/g,'_'); if(k==='p_solicitar')k='can_solicit'; $(i).checked = p[k]||false; });
    $('p-admin').checked = p.admin||false; $('btnSaveUser').innerText = "ACTUALIZAR"; setDisplay('modal-usuario', 'flex');
};

window.resetUserForm = () => {
    $('user-form-title').innerHTML = `<span class="material-icons-round">person_add</span> Nuevo Usuario`;
    ...
    ['p-solicitar','p-ver-propias','p-ver-ger','p-ver-todas','p-paso1','p-paso2','p-paso4','p-gest-sgc','p-ger-apr','p-users','p-struct','p-ver-listado','p-audit-ver','p-audit-admin','p-audit-auditor','p-audit-dueno','p-admin'].forEach(i => $(i).checked=false);
    $('p-ver-propias').checked=true; $('btnSaveUser').innerText = "GUARDAR"; 
};
...
window.agregarColumna = async () => { let v = $('col-nom').value.trim(), t = $('col-tipo').value; if(!v) return; columnasMaestro.push({nombre: v, tipo: t}); await setDoc(doc(db, "artifacts", appId, "public", "data", "Configuracion", "MaestroSettings"), { columnas: columnasMaestro }, {merge: true}); $('col-nom').value = ""; };
...
window.verDetalle = async (id) => {
    ...
    $('m-id').innerText = s.customId; $('m-tit').innerText = s.titulo; $('m-sol').innerText = s.solicitante;
    let est = (s.estado || "").toUpperCase(), apr = est.includes('APROBADO FINAL'), cnc = est==='ANULADO'||est==='RECHAZADO';
    $('m-est').innerText = apr?'APROBADO FINAL':s.estado; $('m-est').className = `badge ${apr?'badge-success':(cnc?'badge-danger':'badge-warning')}`;
    $('m-ger').innerText = s.gerencia; $('m-tipo').innerText = s.tipoDoc || "N/A"; $('m-prioridad').innerText = s.prioridad.toUpperCase(); $('m-prioridad').className = `badge ${s.prioridad==='Alta'?'badge-danger':'badge-info'}`;
    $('m-accion').innerText = s.accion; $('m-jus').innerText = s.motivo || "N/A";
    let dlUrl = s.adjunto ? window.getDownloadUrl(s.adjunto) : "#", an = s.adjunto_nombre || "Adjunto"; $('m-file-link').innerHTML = s.adjunto ? `<a href="#" onclick="window.abrirDocumento('${dlUrl}', '${an}'); return false;" class="file-link">📎 ${an}</a>` : "N/A";
    if(s.accion !== 'Creación') { setDisplay('m-extra-panel', 'block'); $('m-cod').innerText = s.cod_ref; $('m-ver').innerText = s.ver_ref; $('m-fecha-ult').innerText = window.formatearFechaAbreviada(s.fecha_ref); } else setDisplay('m-extra-panel', 'none');
    ...
    if(s.fecha_esperada_cierre) { setDisplay('m-admin-sla','block'); $('m-sla-date').value=s.fecha_esperada_cierre; $('m-sla-date').disabled=!esAdm; setDisplay('btn-save-sla',esAdm?'inline-block':'none'); } else if(esAdm&&act) { setDisplay('m-admin-sla','block'); $('m-sla-date').value=''; $('m-sla-date').disabled=false; setDisplay('btn-save-sla','inline-block'); } else setDisplay('m-admin-sla','none');
    setDisplay('m-panel-final-sgc', 'none'); setDisplay('m-panel-update-sgc', 'none'); setDisplay('m-display-final', 'none'); $('m-original-data').classList.remove('locked-data');
    if((esAdm||p.p_paso2) && s.idx===1 && act) { setDisplay('m-panel-update-sgc','block'); $('m-upd-tit').value=s.titulo; $('m-upd-cod').value=s.cod_ref||''; $('m-upd-ver').value=s.ver_ref||''; }
    if(apr) { if(s.version_final) { $('m-original-data').classList.add('locked-data'); setDisplay('m-display-final','block'); $('m-disp-cod').innerText=s.codigo_final||s.cod_ref; $('m-disp-ver').innerText=s.version_final; $('m-disp-fecha').innerText=window.formatearFechaAbreviada(s.fecha_final); $('m-disp-file').innerHTML=s.documento_final?`<a href="#" onclick="window.abrirDocumento('${s.documento_final}', '${s.documento_final_nombre||'Doc'}');return false;" class="file-link">📄 ${s.documento_final_nombre}</a>`:"N/A"; } else if(esAdm||p.p_paso4) { setDisplay('m-panel-final-sgc','block'); $('m-final-cod').value=s.cod_ref||''; } }
...
window.guardarAuditoria = async () => {
    ...
    if(editandoAuditoriaId) { await updateDoc(doc(db, "artifacts", appId, "public", "data", "Auditorias", editandoAuditoriaId), dt); } 
        ...
        await addDoc(collection(db, "artifacts", appId, "public", "data", "Auditorias"), dt); alert(`Auditoría ${aNum} programada.`);
    }
...
window.renderTablaAuditorias = (yf) => {
    ...
    globalAuditorias.forEach(a => {
        ...
        if(isAdm||(a.auditor&&a.auditor.includes(currentUser.nombre))) { if(e==='Programada') btn+=`<button class="btn btn-success" style="padding:4px;font-size:10px;margin-right:5px;" onclick="window.iniciarAuditoriaDirecto('${a.id}')">Iniciar</button>`; else if(e==='En Progreso') btn+=`<button class="btn btn-warning" style="padding:4px;font-size:10px;margin-right:5px;" onclick="window.finalizarAuditoriaDirecto('${a.id}')">Fin</button>`; btn+=`<button class="btn btn-info" style="padding:4px;font-size:10px;margin-right:5px;" onclick="window.cargarAuditoriaParaEditar('${a.id}')">Ed</button>`; }
...
window.verModalAuditoria = async (id) => {
    ...
    setDisplay('btn-tab-f020', (isAdm||isAud)?'inline-block':'none'); setDisplay('btn-add-f020', canEd?'inline-block':'none'); setDisplay('btn-save-f020', canEd?'inline-block':'none'); setDisplay('btn-save-f003', canEd?'inline-block':'none'); setDisplay('btn-add-sac-manual', canEd?'inline-block':'none');
...
window.renderF020 = () => {
    ...
    currentAuditF020.forEach((i, idx) => {
        let rOpt = `<option value="">-- Sel --</option>` + requisitosOEA.map(r=>`<option value="${r}" ${i.requisito===r?'selected':''}>${r}</option>`).join('');
        let nOpt = `<option value="N/A" ${i.nc==='N/A'?'selected':''}>N/A</option><option value="NC Menor" ${i.nc==='NC Menor'?'selected':''}>NC Menor</option><option value="NC Mayor" ${i.nc==='NC Mayor'?'selected':''}>NC Mayor</option><option value="OM" ${i.nc==='OM'?'selected':''}>OM</option>`;
        let fOpt = `<option value="N/A" ${i.fortaleza==='N/A'?'selected':''}>N/A</option><option value="Sí" ${i.fortaleza==='Sí'?'selected':''}>Sí</option>`;
        h+=`<tr data-id="${i.id}"><td>${idx+1}</td><td><textarea class="table-input" rows="2" ${canEd?'':'disabled'}>${i.pregunta||''}</textarea></td><td><select class="table-select" ${canEd?'':'disabled'}>${rOpt}</select></td><td><textarea class="table-input" rows="2" ${canEd?'':'disabled'}>${i.comentarios||''}</textarea></td><td><input type="text" class="table-input" value="${i.auditado||''}" ${canEd?'':'disabled'}></td><td><select class="table-select hallazgo-sel" ${canEd?'':'disabled'}>${nOpt}</select></td><td><textarea class="table-input" rows="2" ${canEd?'':'disabled'}>${i.observacion||''}</textarea></td><td><select class="table-select" ${canEd?'':'disabled'}>${fOpt}</select></td><td class="f020-action-col">${canEd?`<button class="btn-icon-danger" onclick="window.eliminarF020('${i.id}')"><span class="material-icons-round">delete</span></button>`:''}</td></tr>`;
...
window.renderAuditSACs = () => {
    ...
    hs.forEach((h, idx) => {
        ...
        else { bd=`<span class="badge badge-dark">NO CREADA</span>`; if(currentUser.permisos.p_audit_auditor||currentUser.permisos.admin||(selectedAuditData.auditor&&selectedAuditData.auditor.includes(currentUser.nombre))) btn=`<button class="btn btn-info" style="padding:4px;font-size:10px;" onclick="window.abrirCrearSAC('${h.id}')">CREAR SAC</button>`; }
...
window.verSAC = (id) => {
    ...
    $('sac-fecha').value=sac.fecha_registro||sac.fecha_apertura.split('T')[0]; $('sac-proceso').value=sac.proceso||""; $('sac-tipo').value=sac.tipo_hallazgo||"";
...
window.renderF023Global = () => {
    ...
    fs.forEach(s => {
        ...
        hs+=`<tr><td><b>${s.sac_num}</b></td><td>${s.proceso}</td><td><b style="${s.tipo_hallazgo==='NC Mayor'?'color:var(--danger)':'color:var(--warning)'}">${s.tipo_hallazgo}</b></td><td>${uD?uD.nombre:s.dueno_uid}</td><td><div style="max-width:250px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${s.detalle_nc}</div></td><td>${window.formatearFechaAbreviada(s.fecha_registro||s.fecha_apertura)}</td><td><span class="badge ${bs}">${es}</span></td><td>${s.fecha_cierre?window.formatearFechaAbreviada(s.fecha_cierre):'-'}</td><td class="no-export"><button class="btn btn-primary" style="padding:4px;font-size:10px;" onclick="window.verSACGlobal('${s.sac_id}', '${s.audit_id}')">Ver</button></td></tr>`;
    }); tb.innerHTML=hs;
};
window.setFilterGestNC = () => window.renderF023Global();
window.verSACGlobal = async (sId, aId) => { const sn=await getDoc(doc(db,"artifacts",appId,"public","data","Auditorias",aId)); if(sn.exists()){selectedAuditData=sn.data();selectedAuditId=aId;} window.verSAC(sId); };
window.exportarExcelNoConf = () => { if(globalAllSacs.length===0) return alert("Vacio."); let dE=globalAllSacs.map(s=>{let u=allUsers.find(x=>x.usuario===s.dueno_uid); return {"N° SAC":s.sac_num, "Req":s.proceso, "Tipo Doc":s.tipo_doc_afectado||'N/A', "Tipo":s.tipo_hallazgo, "Resp":u?u.nombre:s.dueno_uid, "Detalle":s.detalle_nc, "Apertura":s.fecha_apertura?new Date(s.fecha_apertura).toLocaleString():'', "Causa":s.causa_raiz||'', "Acción":s.accion_implementar||'', "Estado":s.estado, "Cierre":s.fecha_cierre?new Date(s.fecha_cierre).toLocaleString():'', "Cerrado Por":s.cerrado_por||'' }; }); let wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(dE), "F-023"); XLSX.writeFile(wb, "F-023_Control_NC.xlsx"); };
